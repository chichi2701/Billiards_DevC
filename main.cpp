...
/************************************************************************
  Collisions Checking and Updating (Kiem tra va cap nhat va cham)
 ************************************************************************/
void initBalls(){ //khoi tao bong
	balls[0]->pos = glm::vec3(-0.3, table.heigh + balls[0]->radius, 0);
	balls[0]->vel = glm::vec3(0, 0, 0);	
	balls[0]->isInHole = false;
	balls[1]->pos = glm::vec3(0.1, table.heigh + balls[1]->radius, 0.0);
	balls[1]->vel = glm::vec3(0, 0, 0);		
	balls[1]->isInHole = false;
	balls[2]->pos = glm::vec3(0.15, table.heigh + balls[2]->radius, 0.025);
	balls[2]->vel = glm::vec3(0, 0, 0);		
	balls[2]->isInHole = false;
	balls[3]->pos = glm::vec3(0.15, table.heigh + balls[3]->radius, -0.025);
	balls[3]->vel = glm::vec3(0, 0, 0);		
	balls[3]->isInHole = false;	
	balls[4]->pos = glm::vec3(0.2, table.heigh + balls[4]->radius, 0.025);
	balls[4]->vel = glm::vec3(0, 0, 0);		
	balls[4]->isInHole = false;
	balls[5]->pos = glm::vec3(0.2, table.heigh + balls[5]->radius, 0.0);
	balls[5]->vel = glm::vec3(0, 0, 0);		
	balls[5]->isInHole = false;
	balls[6]->pos = glm::vec3(0.2, table.heigh + balls[6]->radius, -0.025);
	balls[6]->vel = glm::vec3(0, 0, 0);		
	balls[6]->isInHole = false;
	balls[7]->pos = glm::vec3(0.25, table.heigh + balls[7]->radius, 0.05);
	balls[7]->vel = glm::vec3(0, 0, 0);		
	balls[7]->isInHole = false;
	balls[8]->pos = glm::vec3(0.25, table.heigh + balls[8]->radius, 0.0);
	balls[8]->vel = glm::vec3(0, 0, 0);		
	balls[8]->isInHole = false;
	balls[9]->pos = glm::vec3(0.25, table.heigh + balls[8]->radius, -0.05);
	balls[9]->vel = glm::vec3(0, 0, 0);		
	balls[9]->isInHole = false;
	move(0, 0, 0);
} 

void checkInHoles(){ //kiem tra trang thai lo
...
void checkColisions(){ // kiem tra va cham
	for (int i=0; i< numOfBall; ++i){
...
void updateBalls(){ //cap nhat trang thai bong
	float dt= 0.05;
	for (int i=0; i< numOfBall; ++i){													
...
/************************************************************************
  Objects Loading (Tai cac doi tuong)
 ************************************************************************/
		
void loadDoorAndImage()
{
	texture tex;
	string textureName = "pool9.jpg";
...
void loadBalls()
{
	...
	balls[3] = new Ball("resource/Ball13.obj", 0, &textures);
	balls[4] = new Ball("resource/Ball3.obj", 0, &textures);
	balls[5] = new Ball("resource/Ball10.obj", 0, &textures);
	balls[6] = new Ball("resource/Ball3.obj", 0, &textures);
	balls[7] = new Ball("resource/Ball13.obj", 0, &textures);
	balls[8] = new Ball("resource/Ball3.obj", 0, &textures);
	balls[9] = new Ball("resource/Ball13.obj", 0, &textures);	
	initBalls();			
}


/************************************************************************
  Objects Drawing (ve cac doi tuong)
 ************************************************************************/
...
void drawDoorAndImage()
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, (textures.find("pool9.jpg")->second).width, (textures.find("pool9.jpg")->second).height,
				0, GL_RGB, GL_UNSIGNED_BYTE, (textures.find("pool9.jpg")->second).image);
